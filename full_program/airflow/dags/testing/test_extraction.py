...
class TestEIA:
    ...
    def test_api_request_with_latest_end_date_none(self, mock_environment_variables, mock_requests_get, mock_get_latest_end_date, mock_natural_gas_spot_prices_response):
        ''' Test api_request method of EIA class where latest end date is specified for a given dataset key and non-default offset '''
    ...
    def test_get_max_period_with_data(self, mock_natural_gas_spot_prices_response):
        ''' Test get_max_period method of EIA class where data is not None '''
        data = mock_natural_gas_spot_prices_response 
        result = EIA.get_max_period(data=data)
        assert result == '1999-01-05'
    
    def test_get_max_period_with_no_data(self):
